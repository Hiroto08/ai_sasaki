...
本編14枚＋つなぎ7枚＝**全21枚**。動画は5本のまま。

### ★総評スライド（Q3のあと・QRの前）

**会場と佐々木社長に、プロジェクトの継続可否を判断してもらう**クライマックス。
「継続」「中止」の2択を出し、拍手で判定 → 最後に社長へ振る。

文言はファイル先頭の `JUDGE` で変えられる。

```js
const JUDGE = {
  badge: "JUDGE",
  kicker: "総　評",
  ask: "「President-1.0」の開発を\n続行してよいか？",
  options: ["継　続", "中　止"],
  foot: "ご判断は、会場の拍手と 社長の一声で",
};
```
...
**`q` の中の空行で「状況説明」と「問い」に分かれる。**
状況説明は小さめ、問いは大きく、あいだに区切り線が入る。
全部同じ大きさだと行が幅いっぱいまで伸びて、どこで切れているか分からなくなるため。
...
q: "会議室に入ると、一人を除いて\n" +      // ← 状況説明（小さめ）
   ...
   "入ってきました。\n" +
   "\n" +                                  // ← ここで分かれる
   "あなたは開口一番、何と言いますか？"      // ← 問い（大きく）
```

**改行は書いた位置がそのまま出る**（自動の折り返しに任せない）。
1行 **12〜18字**を目安に、文節の切れ目で割ると読みやすい。
