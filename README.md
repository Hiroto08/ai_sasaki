...
| 6.5 | [**docs/show.html**](docs/show.html) | ★★**本番進行ランナー**。タイトル→登場→自己紹介→Q1→Q2→Q3→QR を**1枚のページで通す**（画面切り替えなし）。動画はドロップで割り当て（**名前不問**） |
...
開始画面に**動画をまとめてドロップするだけ**。名前は何でもよく、上から順に割り当たる。
...
毎回ドロップするのが面倒なら、[`docs/video/`](docs/video/README.md) に規定の名前で置けば自動で入る。
...
- 動画が未割当の場面は代役が出るので、**動画が揃う前から通し稽古できる**
