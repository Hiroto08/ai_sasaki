...
### 笑いの濃さを変える（data/config.json の `humor.level`）

余興用なので笑わせてよいが、**優先順位は「本人に似ていること」が上**。
そのため**笑いの素材を本人由来のものだけに限定**している
（へっぴり腰／巨人／ゴルフ／緊張しい／AIであること）。
汎用のギャグ・ダジャレ・人をいじる笑いは禁止（型の詳細は `data/persona.md`「ユーモアの作法」）。
...
| `off` | 笑いを狙わない |
| `mild` | 5回に1回くらい |
| **`normal`** | **3回に1回くらい（現在の設定）** |
| `high` | 2回に1回。ただし相談ごとには乗せない |

> 💡 まじめな相談・落ち込んでいる相手には、どの設定でも笑いを乗せない。
> ここで茶化すと「人たらし」ではなくなるため。
