...
gcloud run deploy ai-shacho \
  ...
  --max-instances 1 \
  --memory 512Mi \
  --set-env-vars "^@^GEMINI_API_KEY=ここにSTEP1のAPIキー@PASSPHRASE=ここに合言葉@SECRET=ここに下の固定値@ADMIN_KEY=ここに管理キー@MODEL=gemini-2.5-flash@MAX_TURNS=30@MAX_PER_MIN=10"
```

> 📋 **`ADMIN_KEY` は応対ログ画面（`/admin.html`）を開くための鍵です。**
> 好きな文字列で構いませんが、**合言葉とは別のもの**にしてください（合言葉は会場全員が知っています）。
> 例：`log-sasaki-0901`。未設定にするとログ画面は開けません（ログ自体は記録されます）。
> 詳しくは **STEP 5.5** を参照。
...
## STEP 5.5：応対ログを見る（`/admin.html`）

**どんな質問が来たか、AIが何と答えたかを、その場で一覧できます。**

1. ブラウザで **`https://（あなたのURL）/admin.html`** を開く
2. デプロイ時に決めた **`ADMIN_KEY`** を入力する
3. 発話数・利用者数・辛口モードの利用回数・APIエラー数・応答時間が並び、
   下に全会話の一覧が出る（キーワードで絞り込み可、新しいものが上）
4. 右上の **「⬇ CSVで保存」** で Excel で開ける形で落とせる

> 🚨 **イベントが終わったら、片付けの前に必ずCSVを保存してください。**
> ログはサーバーのメモリ上にあるため、**コンテナが再起動すると消えます。**
>
> 保存し忘れた場合の保険として、同じ内容が Cloud Logging にも残っています。
> Google Cloud コンソール ≡ →「ログ エクスプローラ」→ 検索欄に `CHATLOG` と入れて
> 期間を当日にすると取り出せます（こちらは再起動しても消えません）。

- **`ADMIN_KEY` は合言葉とは別物にしてください。**合言葉は会場全員が知っています
- URLを知られてもキーがなければ開けません。逆に**キーを配ると誰でも全会話を読めます**
- 記録されるのは「日時・利用者ID・モード・質問・回答」だけです。
  利用者IDはトークンのハッシュ値なので、**誰が書いたかは分かりません**
  （同じ人が続けて話した分をまとめて追うことだけができます）
- 会話内容を保存することは、アプリ冒頭の注記に明記してあります
...
- [ ] 予算アラートのメールが来ていないか確認
- [ ] `/admin.html` が `ADMIN_KEY` で開けるか確認（当日の記録が取れる状態か）
- [ ] **お開き後、`/admin.html` から応対ログのCSVを保存**

## STEP 8：イベント終了後の片付け

> 🚨 **削除する前に、応対ログのCSVを保存してください。**
> `https://（あなたのURL）/admin.html` →「⬇ CSVで保存」（STEP 5.5）。
> **サービスを消すと、メモリ上のログは取り出せなくなります。**
